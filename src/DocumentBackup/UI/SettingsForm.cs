...
public sealed class SettingsForm : Form
{
    ...
    public SettingsForm(AppHost host)
    {
        ...
        layout.Controls.Add(
            new Label { Text = "연결된 저장장치의 .ppt / .pptx (하위 폴더 포함) + 이 PC에서 연 모든 .ppt / .pptx", AutoSize = true },
            1,
            0);
        ...
        layout.Controls.Add(new Label { Text = "원본 읽기 제한 (MiB/s):", AutoSize = true }, 0, 2);
    ...
    private void RefreshGoogle()
    {
        ...
        {
            ConnectionState.Connected => $"연결됨: {account}. 연 발표자료만 이 계정의 '{_host.Settings.DriveFolderName}' 폴더로 올라갑니다.",
