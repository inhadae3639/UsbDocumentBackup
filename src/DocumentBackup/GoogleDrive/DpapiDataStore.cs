...
{
    /// <summary>
    /// Ties the ciphertext to this app, so another program's blob cannot be swapped in.
    ///
    /// The value still carries the application's former name and must stay exactly as it is: it is
    /// part of the key. Changing it would make every token already on disk undecryptable and force
