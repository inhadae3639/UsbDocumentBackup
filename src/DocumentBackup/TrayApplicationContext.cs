...
public sealed class TrayApplicationContext : ApplicationContext
{
    ...
    private void UpdateTrayText()
    {
        ...
        try
        {
            ...
            text = needsReconnect
                ? $"문서 백업 — Google 재연결 필요 (업로드 대기 {status.UploadsWaiting + status.UploadsNeedAttention}건)"
                : status.Paused
                    ? "문서 백업 — 일시정지"
                    : $"문서 백업 — 보관 {status.RetainedComplete}건, 업로드 대기 {status.UploadsWaiting}건";
        }
